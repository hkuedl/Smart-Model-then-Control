...
for d in range(Daily_days):
    for p in range(Daily_opt):
        IC_map = DF_Model_Func.MZ_CvxModel(n_zone = nn_zone, n_feature = predict_period, n_hidden = 32, n_output=1, predict_period = predict_period).to(device)
        m_state_dict = torch.load(str(nn_zone)+'zone_ICNN_model_'+str(d)+'_'+str(p)+'.pt')
        IC_map.load_state_dict(m_state_dict)
        IC_map_all.append(IC_map)
        IC_input_01_all.append(np.load('IC_input_'+str(nn_zone)+'zone_'+str(d)+'_'+str(p)+'.npy'))
...
class MZ_opt_joint_map_all(nn.Module):
    def __init__(self, P_models,M_model,IC_map_all):
        ...
        self.lstm_list = nn.ModuleList([DF_Model_Func.LSTM(1, 20, 3, predict_period) for _ in range(len(P_models))])
    ...
    def forward(self, Dis):
        ...
        c_p_F = torch.zeros(D_pred[0].shape[0],D_pred[0].shape[1],90).to(device)
        for i in range(D_pred[0].shape[1]):
            input = self.lstm_list[0](Dis[0])[:,i:i+1].to(device)
            for ii in range(1,2*nn_zone+1):
                input = torch.cat((input,self.lstm_list[ii](Dis[ii])[:,i:i+1]),dim = 1).to(device)   #样本数量*3
...
for i in range(S_X_tr.shape[-1]):
    if i in [99,108,117,126,135,144,153,162,171,180]:
...
for epoch in range(J_epoch):
    for num in range(1):  #(J_batch_num):
        ...
        J_cpA = J_model.model.a.weight   #.state_dict()['weight']
        ...
        for c_i_index in range(len(J_batch_list_i)):
            ...
            c_obj1_i, c_obj2_i, c_obj_i = DF_Model_Func.MZ_opt_objective(T_Fre,SS_P,c_period_i, J_price, c_solution[1],c_solution[2],predict_period)  #然后计算目标函数
...
I_model = -1
for i in range(S_X_tr.shape[-1]):
   torch.save(J_models[I_model][i], 'Joint_'+str(nn_zone)+'zone_LSTM_'+str(i)+'_nn.pt')
torch.save(J_models[I_model][-1], 'Joint_'+str(nn_zone)+'zone_Model_nn.pt')


J_lstm_n = []
for i in range(S_X_tr.shape[-1]):
    J_lstm_n.append(DF_Model_Func.LSTM(P_nn_input,P_nn_hidden,P_nn_layer,P_nn_output).to(device))
    J_lstm_n[i].load_state_dict(torch.load('Joint_'+str(nn_zone)+'zone_LSTM_'+str(i)+'_nn.pt'))
...
J_model_n.load_state_dict(torch.load('Joint_'+str(nn_zone)+'zone_Model_nn.pt'))

M_train_data,M_train_label = DF_Model_Func.MZ_model_data(S_X_tr,S_P_tr,S_Y_tr, train_period, predict_period)
M_test_data,M_test_label = DF_Model_Func.MZ_model_data(S_X_te,S_P_te,S_Y_te, train_period, predict_period)

M_test_pred = to_np(J_model_n(M_test_data[:-1,:,:])[0])
...
M_train_pred = to_np(J_model_n(M_train_data[:-1,:,:])[0])
...
for c_day_index in range(0, Daily_days+Daily_days_te):
    for c_prd_index in range(Daily_opt):
        ...
        Cr_obj[c_i_index,:] = [c_prob.value,c_obj1.value[0,0],c_obj2.value]
...
import pandas as pd
writer = pd.ExcelWriter('Joint_'+str(nn_zone)+'zone_opt.xlsx')
