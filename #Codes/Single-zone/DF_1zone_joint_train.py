...
for d in range(Daily_days):
    for p in range(Daily_opt):
        if CVXNN == 1:
            IC_map = DF_Model_Func.CvxModel(n_feature = predict_period, n_hidden = 64, n_output=1).to(device)
            m_state_dict = torch.load('2zone_ICNN_model_'+str(d)+'_'+str(p)+'.pt')
        else:
            IC_map = DF_Model_Func.FNNModel(n_feature = predict_period, n_hidden = 64, n_output=1).to(device)
            m_state_dict = torch.load('2zone_ICNN_model_'+str(d)+'_'+str(p)+'.pt')
        IC_map.load_state_dict(m_state_dict)
        IC_map_all.append(IC_map)

        IC_input_01_all.append(np.load('IC_input_'+str(d)+'_'+str(p)+'.npy'))

J_epoch = 40
J_epoch_freq = 1
J_train_samples = int((Train_e-Train_s)/predict_period)
J_batch_size = J_train_samples
J_batch_num = math.ceil(J_train_samples/J_batch_size)
J_model = DF_Model_Func.opt_joint_map_all(P_models,M_model,IC_map_all).to(device)
...
J_price,J_upper_tem,J_lower_tem,J_upper_q,J_lower_q = DF_Model_Func.opt_settings(nn_zone,c_0_tem, SS_P,SS_Y, Train_s,Train_e,Train_s2,Train_e2,predict_period,T_Fre)
...
for epoch in range(J_epoch):
    ...
    for num in range(J_batch_num):
        J_batch_size_i = min(J_batch_size,len(J_batch_list))
        J_batch_list_i = random.sample(J_batch_list,J_batch_size_i)
        J_batch_list = [x for x in J_batch_list if x not in J_batch_list_i]
        J_optimizer.zero_grad()
        
        J_batch_data_1 = Dis_tr_data[0][J_batch_list_i,:,:]
        J_batch_data_2 = Dis_tr_data[1][J_batch_list_i,:,:]
        J_batch_data_3 = Dis_tr_data[2][J_batch_list_i,:,:]
        J_batch_label_1 = Dis_tr_label[0][J_batch_list_i,:]
        J_batch_label_2 = Dis_tr_label[1][J_batch_list_i,:]
        J_batch_label_3 = Dis_tr_label[2][J_batch_list_i,:]
        J_batch_dis_pred,J_cpF = J_model(J_batch_data_1,J_batch_data_2,J_batch_data_3)
        J_batch_dis_pred[0].to(device)
        J_batch_dis_pred[1].to(device)
        J_batch_dis_pred[2].to(device)
        J_cpF.to(device)
        
        J_obj_list = []
        J_tem = torch.zeros((J_batch_size_i,predict_period+1)).to(device)
        J_tem_re_value = torch.zeros((J_batch_size_i,predict_period)).to(device)
        J_tem_pen = torch.zeros((J_batch_size_i,predict_period+1)).to(device)
        J_q = torch.zeros((J_batch_size_i,predict_period)).to(device)
        ...
        for c_i_index in range(len(J_batch_list_i)):
            ...
            c_v_tem,c_v_q,c_v_tem_u,c_p_A,c_p_B,c_p_F,c_prob,c_layer,c_obj1,c_obj2 = DF_Model_Func.opt_problem(J_price,J_upper_tem,J_lower_tem,J_upper_q,J_lower_q,SS_P, SS_Y, predict_period,T_Fre,c_period_i)
            c_solution = c_layer(J_model.model.a,J_model.model.b,J_cpF[c_i_index,:])
            c_obj1_i, c_obj2_i, c_obj_i = DF_Model_Func.opt_objective(T_Fre,SS_P,c_period_i, J_price, c_solution[1],c_solution[2],predict_period)
            J_obj_list.append(c_obj_i)
            J_tem[c_i_index,:] = c_solution[0]
            J_q[c_i_index,:] = c_solution[1]
            J_tem_pen[c_i_index,:] = c_solution[2]

            for ii in range(J_tem_re_value.shape[1]):
                J_tem_i = J_tem[c_i_index,ii+1]*(SS_Ymax-SS_Ymin)+SS_Ymin
                J_tem_re_value[c_i_index,ii] = (J_tem_i - IC_input_01_all[c_period_i][0,ii])/(IC_input_01_all[c_period_i][1,ii]-IC_input_01_all[c_period_i][0,ii])
            
            if CVXNN == 1:
                _,_,_,J_true_cost[c_i_index,0] = J_model.ICNN[c_period_i](J_tem_re_value[c_i_index,:])
            else:
                J_true_cost[c_i_index,0] = J_model.ICNN[c_period_i](J_tem_re_value[c_i_index,:])
        J_loss_dis_i = (1/3)*(J_lossfn(J_batch_label_1,J_batch_dis_pred[0]) + J_lossfn(J_batch_label_2,J_batch_dis_pred[1]) + J_lossfn(J_batch_label_3,J_batch_dis_pred[2]))
    ...
    J_models.append([copy.deepcopy(J_model.lstm1.state_dict()), copy.deepcopy(J_model.lstm2.state_dict()), copy.deepcopy(J_model.lstm3.state_dict()), \
...
print('Average Training time: {}'.format((time2-time1)/(epoch+1)))
...
J_model_n = DF_Model_Func.model_FNN(M_nn_input, M_nn_hidden, M_nn_output, M_layers).to(device)
J_model_n.load_state_dict(J_models[I_model][3])

P_disturb = ['out','rad','occ']
P_train_01 = np.zeros((X_tr.shape[0]-train_period,X_tr.shape[1]))
P_train_acc = np.zeros((3,3))
for P_i in [0,1,2]:
    P_train_01[:,P_i:P_i+1] = to_np(J_lstm_n[P_i](Dis_tr_data[P_i])).reshape(-1,1)

P_train_huanyuan = SS_X.inverse_transform(P_train_01)
for P_i in [0,1,2]:
    P_train_acc[P_i,:] = DF_Model_Func.LSTM_acc(P_train_huanyuan[:,P_i:P_i+1],X_tr[train_period:,P_i:P_i+1])

P_test_01 = np.zeros((X_te.shape[0]-train_period,X_te.shape[1]))
P_test_acc = np.zeros((3,3))
for P_i in [0,1,2]:
    P_test_01[:,P_i:P_i+1] = to_np(J_lstm_n[P_i](Dis_te_data[P_i])).reshape(-1,1)

P_test_huanyuan = SS_X.inverse_transform(P_test_01)
for P_i in [0,1,2]:
    P_test_acc[P_i,:] = DF_Model_Func.LSTM_acc(P_test_huanyuan[:,P_i:P_i+1],X_te[train_period:,P_i:P_i+1])

print('1st Train RMSE: {},Test RMSE: {}'.format(P_train_acc[0,0],P_test_acc[0,0]))

for i in range(3):
    torch.save(J_models[I_model][i], 'Joint_2zone_LSTM_'+str(i)+'.pt')
torch.save(J_models[I_model][-1], 'Joint_2zone_Model.pt')

#%%
M_train_data,M_train_label = DF_Model_Func.model_data(S_X_tr,S_P_tr,S_Y_tr, train_period, predict_period)
M_test_data,M_test_label = DF_Model_Func.model_data(S_X_te,S_P_te,S_Y_te, train_period, predict_period)

M_test_pred = to_np(J_model_n(M_test_data[:-1,:,:]))
...
M_train_pred = to_np(J_model_n(M_train_data[:-1,:,:]))
...
for c_day_index in range(0, Daily_days+Daily_days_te):
    for c_prd_index in range(Daily_opt):
        ...
        c_v_tem,c_v_q,c_v_tem_u,c_p_A,c_p_B,c_p_F,c_prob,c_layer,c_obj1,c_obj2 = DF_Model_Func.opt_problem(J_price,J_upper_tem,J_lower_tem,J_upper_q,J_lower_q,SS_P, SS_Y, predict_period,T_Fre, c_i_index)
        c_p_A.value = J_model_n.a.item()
        c_p_B.value = J_model_n.b.item()
        ...
        c_p_F.value = to_np(J_model_n.net(Seq_output))[:,0]
...
#%%
import pandas as pd
writer = pd.ExcelWriter('Joint_2zone_opt.xlsx')
